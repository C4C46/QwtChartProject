	...
	m_configLoader(configLoader)
{
	...
	if (!selectedParentNames.isEmpty())
	{
		...
		if (settingDefaults.contains("yAxisRange"))
		{
			...
			if (yAxisRange.size() == 2) {
				double maxY = yAxisRange[0].toDouble();
				double minY = yAxisRange[1].toDouble();
	...
	// 设置X轴和Y轴的初始范围
	m_plot->setAxisScale(QwtPlot::xBottom, 0, XAISX_RANGE); // 设置X轴的范围为0-50
	...
	connect(m_configLoader, &lpTendencyChartConfig::sgAllCurveNames, this, &lpTendencyDataChart::onAllCurveNames);
	installEventFilters();
...
lpTendencyDataChart::~lpTendencyDataChart() {
	...
	m_updateTimer->stop();
    m_alarmMarkerLower->detach();
    m_alarmMarkerUpper->detach();
    delete m_alarmMarkerLower;
    delete m_alarmMarkerUpper;
    m_alarmMarkerLower = nullptr;
    m_alarmMarkerUpper = nullptr;
...
void lpTendencyDataChart::onChartUpdate(const QString &curveName, double x, double y) {
	...
	if (x - m_lastResetX >= METER_THRESHOLD) {
		...
		m_plot->setAxisScale(QwtPlot::xBottom, m_lastResetX, m_lastResetX + XAISX_RANGE); // 重置X轴的范围为当前x到x+50
		m_plot->replot();
	...
	if (!m_isViewingHistory) {
		xInterval = XAISX_RANGE; // 这里设置x轴的间隔值，根据实际情况调整
	...
	// 保持滑块在最右端
	if (m_slider->value() == m_slider->maximum() - XAISX_RANGE) { // 假设显示范围是50
...
void lpTendencyDataChart::initAlarmPlot()
{
    QPen pen;
    ...
    m_alarmMarkerUpper = new QwtPlotMarker();
    m_alarmMarkerUpper->setLineStyle(QwtPlotMarker::HLine);
    m_alarmMarkerUpper->setLinePen(Qt::red, 3, Qt::DashLine);
    m_alarmMarkerUpper->attach(m_plot);

    m_alarmMarkerLower = new QwtPlotMarker();
    m_alarmMarkerLower->setLineStyle(QwtPlotMarker::HLine);
    m_alarmMarkerLower->setLinePen("#FF6666", 3, Qt::DashLine);
    m_alarmMarkerLower->attach(m_plot);

    QString alarmparentnames = m_configLoader->getSelectedParentNames();
    if (!alarmparentnames.isEmpty())
    {
        QVariantMap settingDefaults = m_configLoader->getSettingDefaultValue(alarmparentnames);
        if (settingDefaults.contains("alarmValue"))
        {
            QVariantList alarmValue = settingDefaults["alarmValue"].toList();
            updateAlarmValue(alarmValue);
            m_plot->replot();
        }
    }
...
void lpTendencyDataChart::onAllCurveNames(const QStringList &allcurveNames)
{
    // 清除现有的曲线和数据
	for (auto &curve : m_curves) {
		curve->detach(); // 从图表中移除曲线
	}
	m_curves.clear(); // 清空曲线列表
	m_xDataMap.clear(); // 清空X数据映射
	m_yDataMap.clear(); // 清空Y数据映射
    m_alarmMarkerLower->detach();
    m_alarmMarkerUpper->detach();
    m_curveNames = m_configLoader->getCurveNames();
...
void lpTendencyDataChart::updateAlarmValue(const QVariantList &alarmValue) {
	if (alarmValue.size() == 2) {
        m_alarmValueUpper = alarmValue[0].toDouble();
        m_alarmValueLower = alarmValue[1].toDouble();

        m_alarmMarkerLower->setYValue(m_alarmValueLower);
        QwtText lowerLabel(QString::number(m_alarmValueLower));
        lowerLabel.setColor("#FF6666");
        QFont lowerfont = lowerLabel.font();
        lowerfont.setPointSize(12);
        lowerLabel.setFont(lowerfont);
        m_alarmMarkerLower->setLabel(lowerLabel);
        m_alarmMarkerLower->setLabelAlignment(Qt::AlignRight | Qt::AlignTop);

        m_alarmMarkerUpper->setYValue(m_alarmValueUpper);
        QwtText upperLabel(QString::number(m_alarmValueUpper));
        upperLabel.setColor(Qt::red);
        QFont upperfont = upperLabel.font();
        upperfont.setPointSize(12);
        upperLabel.setFont(upperfont);
        m_alarmMarkerUpper->setLabel(upperLabel);
        m_alarmMarkerUpper->setLabelAlignment(Qt::AlignRight | Qt::AlignBottom);
...
void lpTendencyDataChart::onSliderValueChanged(int value)
{
	...
	double xMin = value;
	double xMax = xMin + XAISX_RANGE;  // 假设一次显示50m的数据
...
void lpTendencyDataChart::updateSliderPosition() {
	...
	m_slider->setMaximum(xMaxCurrent - XAISX_RANGE); // 假设显示范围是50，确保滑块不会超出当前显示的最大范围
...
void lpTendencyDataChart::clearChart()
{
	...
	m_maxReceivedX = 0;
	// 重置x轴和y轴的范围
	m_plot->setAxisScale(QwtPlot::xBottom, 0, XAISX_RANGE); // 重置X轴的范围为0-50
