...
public:
	...
	void saveConfig(); // 添加保存配置的声明
    void saveLithiumChannelSplitConfig();
    void addChannelItem(QJsonObject * channelObject, const QString & channel, const QJsonObject & item);
...
public slots:
	void updateChildNames(const QStringList &newNames);
	void onCombinedValues(const QMap<QString, QVariantMap> &combinedValues);
...
signals:
	...
	void sgAllCurveNames(const QStringList &allcurveNames);

private:
	QTreeWidget *m_treeWidget;
	QStringList m_curveNames; // 存储曲线名称
	QButtonGroup *m_buttonGroup; // 管理单选按钮的按钮组
     QJsonDocument m_configDoc; // 添加保存配置文件信息成员变量
	 QStringList m_outputNames;  // 用于存储生成的名称
     QString m_jsonString;//保存event.json文件内容
};
